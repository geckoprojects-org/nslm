...
**`cpool`** (the only technique of the Java 21 variant, selectable on Java 25 with `spi.weaver.technique=cpool`) patches only the constant pool, without any bytecode library:
...
**`callsite`** (the default of the Java 25 weaver) rewrites the instructions with the Class-File API (`java.lang.classfile`, Java 24+, no ASM). It is about 1 µs per call cheaper than `cpool` because the caller comes as a constant instead of from a stack walk (see [Performance](#performance)). It lives in `CallSiteWeaver`, which the hook loads by name only when this technique is configured and which is left out of the Java 21 variant (below); when it cannot be loaded, the hook falls back to `cpool` and traces why.
...
- `JdkFactoryPitfallsTest`: the same kind of JDK internal lookup for StAX (Woodstox) and JAXP DOM and SAX (`org.example.serviceloader.jaxp.provider`, delegating to the JDK's built-in parsers, installed by the test), on the test thread, in the common pool and on a thread without TCCL. The weaver passes all of them through `JdkFactories`; the other deployments return the JDK default off the test thread (asserted as known limitation).
- `ProviderClassNameClashTest` (osgi/osgi#970, Tom Watson's comment on the TCCL): two provider bundles ship a provider class of the same name for Greeter 1.0 and 2.0, two callers call `ServiceLoader.load(Class)` with the inherited TCCL or with their own bundle class loader as TCCL. A TCCL shared by all bundles is the JVM's initiating loader of the provider class: the second caller gets `not a subtype`, and after a reinstall the class of the uninstalled bundle keeps coming back. Passes with the weaver. Under the other deployments the inherited TCCL cases and the bundle class loader case after only the provider was replaced (the consumer's loader would need a refresh dependency on the provider) are known limitations, asserted as observed.
...
These three tests assert the correct result where a deployment gets it right and exactly the observed result, with its reason, where it has a known limitation (`Expectation`, `Deployment`: each bndrun sets `test.deployment`; the weaver runs form one group, the launcher based mediator runs and the Equinox extension the other, and within a group all runs behave the same). The build is green, and a fixed or a new limitation shows up as a failing test.
